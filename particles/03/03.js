...
Elm.Main.make = function (_elm) {
   ...
   var fromSimulation = function (s) {
      return {ctor: "_Tuple2"
             ,_0: s.socialMode
             ,_1: function (_p0) {
                return $Html.fromElement(A3($Graphics$Collage.collage,s.width,s.height,A2($List.map,drawParticle,function (_) {    return _.particles;}(_p0))));
             }(s)};
   };
   var moveParticle = F2(function (_p1,p) {
      var _p2 = _p1;
      var _p4 = _p2._0;
      var _p3 = _p2._1;
      var y = $Basics.toFloat(A2($Basics._op["%"],$Basics.round(5 * $Basics.sin($Basics.degrees(p.d)) + p.y) + (_p3 / 2 | 0),_p3) - (_p3 / 2 | 0));
      var x = $Basics.toFloat(A2($Basics._op["%"],$Basics.round(5 * $Basics.cos($Basics.degrees(p.d)) + p.x) + (_p4 / 2 | 0),_p4) - (_p4 / 2 | 0));
      return _U.update(p,{x: x,y: y});
   });
   var rand = A2($Random.$int,-30,30);
   var rotateParticle = F2(function (p,_p5) {
      var _p6 = _p5;
      var _p7 = A2($Random.generate,rand,_p6._1);
      var d = _p7._0;
      var seed = _p7._1;
      return {ctor: "_Tuple2",_0: A2($List._op["::"],_U.update(p,{d: $Basics.toFloat(A2($Basics._op["%"],$Basics.round(p.d) + d,360))}),_p6._0),_1: seed};
   });
   var update = F2(function (op,simul) {
      var _p8 = op;
      switch (_p8.ctor)
      {case "Move": return simul.socialMode ? _U.update(simul,
           {particles: A2($List.map,moveParticle({ctor: "_Tuple2",_0: simul.width,_1: simul.height}),simul.particles)}) : simul;
         case "Rotate": return simul.socialMode ? _U.update(simul,
           {particles: $Basics.fst(A3($List.foldr,rotateParticle,{ctor: "_Tuple2",_0: _U.list([]),_1: $Random.initialSeed(_p8._0)},simul.particles))}) : simul;
         case "Toggle": return _U.update(simul,{socialMode: _p8._0});
         default: return _U.update(simul,{width: _p8._0});}
   });
   var Resize = function (a) {    return {ctor: "Resize",_0: a};};
   var Toggle = function (a) {    return {ctor: "Toggle",_0: a};};
   var view = F2(function (addr,_p9) {
      var _p10 = _p9;
      var _p11 = _p10._0;
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("simulation")]),_U.list([_p10._1]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.classList(_U.list([{ctor: "_Tuple2",_0: "controls",_1: true},{ctor: "_Tuple2",_0: "activated",_1: _p11}]))]),
              _U.list([A2($Html.label,_U.list([A2($Html$Events.onClick,addr,Toggle($Basics.not(_p11)))]),_U.list([$Html.text("on")]))
                      ,A2($Html.label,_U.list([A2($Html$Events.onClick,addr,Toggle($Basics.not(_p11)))]),_U.list([$Html.text("off")]))
                      ,A2($Html.input,_U.list([$Html$Attributes.type$("button"),A2($Html$Events.onClick,addr,Toggle($Basics.not(_p11)))]),_U.list([]))]))]));
   });
   var Rotate = function (a) {    return {ctor: "Rotate",_0: a};};
   var Move = {ctor: "Move"};
   var Particle = F3(function (a,b,c) {    return {x: a,y: b,d: c};});
   var Simulation = F4(function (a,b,c,d) {    return {particles: a,socialMode: b,width: c,height: d};});
   var init = A4(Simulation,A2($List.repeat,300,A3(Particle,0,0,0)),false,500,500);
   var main = function () {
      var operations = $Signal.mailbox(Toggle(false));
      var signals = $Signal.mergeMany(_U.list([A2($Signal.map,Resize,$Window.width)
                                              ,operations.signal
                                              ,A2($Signal.map,
                                              function (_p12) {
                                                 return Rotate($Basics.round($Time.inMilliseconds(_p12)));
                                              },
